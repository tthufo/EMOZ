...
+ (BOOL)addSystem:(NSDictionary*)dict
{
    ...
    if (count > 0)
    {
        VideoRecord *s = [[self getFormat:@"name=%@" argument:@[dict[@"name"]]] lastObject];
    ...
    VideoRecord *b = [NSEntityDescription insertNewObjectForEntityForName:@"VideoRecord" inManagedObjectContext: [db managedObjectContext]];
...
+ (void)modify:(NSString*)vid
{
    ...
    VideoRecord * r = [[self getFormat:@"vid=%@" argument:@[vid]] lastObject];
...
{
    VideoRecord * s = [[self getFormat:@"name=%@" argument:[NSArray arrayWithObjects:key, nil]] lastObject];
...
+ (NSArray *)getFormat:(NSString *)format argument:(NSArray *)argument
{
    ...
    NSEntityDescription *ed = [NSEntityDescription entityForName:@"VideoRecord" inManagedObjectContext:[db managedObjectContext]];
...
+ (void)clearAll
{
    ...
    for(VideoRecord * s in [VideoRecord getAll])
    {
        [[db managedObjectContext] deleteObject:s];
    }
...
+ (void)clearFormat:(NSString *)format argument:(NSArray *)argument
{
    ...
    NSArray * s = [VideoRecord getFormat:format argument:argument];
    
    for(VideoRecord * k in s)
    {
