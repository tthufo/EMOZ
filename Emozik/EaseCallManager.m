...
- (void)makeCall:(NSNotification *)notify
{
    if (notify.object) {
        ...
        [self makeCallWithUsername:[notify.object valueForKey:@"chatter"] isVideo:[[notify.object objectForKey:@"type"] boolValue] andName:[notify.object valueForKey:@"info"]];
    }
}

- (void)makeCallWithUsername:(NSString *)aUsername
                     isVideo:(BOOL)aIsVideo andName:(NSDictionary*)name
{
        ...
        if (strongSelf) {
            ...
            if (!aError && aCallSession) {
                ...
                if (strongSelf.callController == nil) {
                    ...
                    strongSelf.callController = [[EaseCallViewController alloc] initWithCallSession:self.callSession isCaller:YES status:@"Calling" andName:name];
//                    strongSelf.callController.nameLabel.text = name;
                    [strongSelf.rootVc presentViewController:self.callController animated:YES completion:nil];
                } else {
...
- (void)callDidReceive:(EMCallSession *)aSession
{
    ...
    if (_callSession) {
        ...
        [self.rootVc presentViewController:_callController animated:YES completion:^{
            
            if([self.rootVc isVideo])
            {
                [[self.rootVc VIDEO] callIn];
            }
            else if([self.rootVc isKaraoke])
            {
                [[self.rootVc KARAOKE] callIn];
            }
            else
            {
                [[self.rootVc PLAYER] callIn];
            }
