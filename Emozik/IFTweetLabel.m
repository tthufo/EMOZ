...
static NSMutableArray *expressions = nil;

+ (void)initialize
{
    expressions = [@[@"(@[a-zA-Z0-9_]+)",
                    @"(#[a-zA-Z0-9_-]+)",
                    @"([hH][tT][tT][pP][sS]?:\\/\\/[^ ,'\">\\]\\)]*[^\\. ,'\">\\]\\)])"] mutableCopy];
}

- (void)customTag:(NSArray*)tags
{
    [expressions addObjectsFromArray:tags];
}
...
@implementation NSMutableAttributedString (colorChange)
