...
- (instancetype)initWithCallSession:(EMCallSession *)session isCaller:(BOOL)isCaller status:(NSString *)status andName:(NSDictionary*)name
{
...
- (void)didRequestFriend
{
    [[LTRequest sharedInstance] didRequestInfo:@{@"CMD_CODE":@"getlistfriend",
                                                 ...
                                                 @"overrideAlert":@(1),
                                                 @"overrideOrder":@(1),
                                                 @"postFix":@"getlistfriend"} withCache:^(NSString *cacheString) {
                                                     ...
                                                     if(isValidated)
                                                     {
                                                         ...
                                                         for(NSDictionary * dict in [data lastObject][@"LIST"])
                                                         {
                                                             if([[dict getValueFromKey:@"ID"] isEqualToString:_callSession.remoteName])
                                                             {
                                                                 [self.avatarView imageWithUsername:profileInfo[@"uName"] placeholderImage:[UIImage imageNamed:@"default_avatar"] andInfo:@{@"notMine":dict[@"AVATAR"]}];
...
- (void)setupSubViews
{
    ...
    self.nameLabel.text = profileInfo[@"uName"];//_callSession.remoteName;
...
- (void)reloadCallingUI
{
    ...
    [self.avatarView imageWithUsername:profileInfo[@"uName"] placeholderImage:[UIImage imageNamed:@"default_avatar"] andInfo:profileInfo];
    if (_callSession.type == EMCallTypeVideo) {
...
- (IBAction)answerCallAction:(UIButton *)sender {
    ...
    if (_isCaller) {
        ...
        NSString *username = [profileInfo[@"uName"] copy];
...
- (void)close
{
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:KNOTIFICATION_CALL_VIDEO object:nil];
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        [self dismissViewControllerAnimated:YES completion:^{
            ...
            if(![self isVideo])
            {
                [[self PLAYER] didReEnableSession];
                
                [[self PLAYER] didResumePlaying];
            }
            else if([self isVideo])
            {
                [[self VIDEO] didReEnableSession];
                
                [[self VIDEO] didResumePlaying];
            }
            else
            {
                [[self KARAOKE] didReEnableSession];
                
//                [[self KARAOKE] didResumePlaying];
            }
        }];
    });
