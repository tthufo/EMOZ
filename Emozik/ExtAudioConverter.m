...
void startConvertMP3(ExtAudioConverterSettings* settings, ExtAudioConverter * converter){
    ...
    while (1) {
        ...
        if (framesCount==0) {
            ...
            converter.convertCompletion(@{@"code":@(1),@"error":@"Conversion Success"});
            return;
        }
...
@synthesize convertCompletion;

- (void)didStartConvert:(ConvertCompletion)convert
{
    self.convertCompletion = convert;
    [self convert];
}
...
- (BOOL)convert{
    ...
    if (self.inputFile==NULL) {
        ...
        self.convertCompletion(@{@"code":@(0),@"error":@"ConversionFailed"});
        return NO;
    }
